...
if ( ! defined('__CPC_LOGGER_LOG_PHP') )
{
	...
	class Logger_Log {
		...
		static function debug($msg, $ctx = null) {
			$includeCtx = array();
			$excludeCtx = array("__AuthZServer_autoload", "__CPC_autoload", "Core_User");
			$excludeCtx = array_merge($excludeCtx, array("EPLc_Manager", "padmanager.php", 
								"Service_add", "Service_remove", "Service_groupsession",
									"Service_grouppadlist", 
								"EPLc_Service_Registry"));
		...
		static function warn($msg, $ctx = null) {
			echo "warn:" . ($ctx!=''?$ctx.':':'') . $msg . "<br/>\n";
				
		}

		static function error($msg, $ctx = null) {
			echo "ERROR:" . ($ctx!=''?$ctx.':':'') . $msg . "<br/>\n";
				
		}

		static function fatal($msg, $ctx = null) {
			echo "FATAL:" . ($ctx!=''?$ctx.':':'') . $msg . "<br/>\n";
